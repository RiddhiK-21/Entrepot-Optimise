...
def home(request):
	...
	if request.method=="POST":
		...
		if pname in A:
			racks=VaccantRacks.objects.filter(category='A')
			num_bins = len(racks)
			data['bins'] = list(range(num_bins))
			data['bin_capacities']=[int(i.capacity) for i in racks]
			nbin=[]
			nbin=[str(i.rackname) for i in racks]
			ll=list(zip(data['bin_capacities'],nbin))
		elif pname in B:
			racks=VaccantRacks.objects.filter(category='B')
			num_bins = len(racks)
			data['bins'] = list(range(num_bins))
			data['bin_capacities']=[int(i.capacity) for i in racks]
			nbin=[]
			...
			racks=VaccantRacks.objects.filter(category='C')
			num_bins = len(racks)
			data['bins'] = list(range(num_bins))
			data['bin_capacities']=[int(i.capacity) for i in racks]
			nbin=[]
		...
		num_bins = len(all_vaccantracknames)
		data['bins'] = list(range(num_bins))
		data['bin_capacities']=[int(i.capacity) for i in all_vaccantracknames]
		
		nbin=[]
		
		nbin=[str(i.rackname) for i in all_vaccantracknames]
		...
		if status == pywraplp.Solver.OPTIMAL:
			...
			for j in data['bins']:
				...
				r=VaccantRacks.objects.filter(rackname=rname)
				cap=int(r.capacity)-bin_weight
				VaccantRacks.objects.filter(rackname=rname).update(capacity=cap)
				print()
